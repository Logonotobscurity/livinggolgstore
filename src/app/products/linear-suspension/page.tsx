...
export const metadata: Metadata = {
  ...
  description: 'Discover modern and elegant linear suspension lighting to illuminate kitchen islands, dining tables, and more.',
};

export default function LinearSuspensionPage() {
  const products = PlaceHolderImages.filter(p => p.id.startsWith('linear-suspension-'));
  const heroImage = products.length > 0 ? products[0] : PlaceHolderImages.find(p => p.id === 'linear-suspension-hero');
